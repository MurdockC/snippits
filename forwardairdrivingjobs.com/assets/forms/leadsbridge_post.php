...
if( empty($errors) && empty($bot) ) // Checks to see if there are errors and that the 'content' field is not filled in
	{
		...
		if( empty($companyID) ) {
		    ...
		    // FORM REACTOR - SEND FORM TO CALL TRACKING METRICS FOR PHONE CALLS
				...
				if (strpos($source, 'socialedge_ltl') !== false) {
			...
			$message .= '</table>';
			$message .= '<p style="padding-right: 5px; padding-left: 5px; padding-top: 20px; padding-bottom: 20px; text-align: left; background-color: #fff;" align="left" bgcolor="#fff" font-size:18px;><strong>SocialEdge cannot require fields, some may be left blank.</strong></p>';
			$message .= '</body></html>';
